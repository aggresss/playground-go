...
// Fork from https://github.com/quic-go/quic-go/tree/master/internal/utils/ringbuffer
//
...
func (r *RingBuffer[T]) Offset(index int) *T {
	if r.Empty() || index >= r.Len() {
		panic("offset from invalid index")
	}
...
func (r *RingBuffer[T]) Front() *T {
	if r.Empty() {
		panic("front from an empty queue")
	}
...
func (r *RingBuffer[T]) Back() *T {
	if r.Empty() {
		panic("back from an empty queue")
	}
